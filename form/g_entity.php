<html>
    <head>
        ...
        $cols = $centity->itementity($gentity,1);
        // Campos de filtro
        $afilter = $centity->itementity($gentity,2);
        // Función busquedagrid
        if ($_POST['idform'] == 'ffindentity') {
            $rows = $centity->postauto($gentity);
        }else {
...
//            }else {
//                // Lanzar la busqueda
//                $rows = $centity->getbysearch("", "", $gentity);
            }
        } 
        ?>
    </head>
    <body>
        <form name="ffindentity" id="ffind" method="post">
           <!--Parametro oculto que identificar el form.--> 
           <input type="hidden" name="idform" value="ffindentity">
            <div id="dfind">
                <?php
                    ...
                    foreach($afilter as $filtro)
                    {
                        ...
                        if ($acol['type'] <> 'date') {
                            $centity->labelinput($acol['name'],"",$acol['label'],$acol['type'],$acol['size'],$acol['brequeried'],$acol['bfind'],false);
                               ...
                               $centity->labelinput('D_'.$acol['name'],"",'D. '.$acol['label'],$acol['type'],$acol['size'],$acol['brequeried'],false,false); 
                               $centity->labelinput('H_'.$acol['name'],"",'H. '.$acol['label'],$acol['type'],$acol['size'],$acol['brequeried'],$acol['bfind'],false); 
                        }
                    }
